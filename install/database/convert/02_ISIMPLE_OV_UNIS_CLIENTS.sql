CREATE OR REPLACE VIEW ISIMPLE_OV_UNIS_CLIENTS AS
SELECT
    -- PK
    CUS.icusnum                                 AS CUSNUM,
    CAST(u.cclientid         AS VARCHAR2(64))   AS UNIS_CLIENT_ID,
    ...
    CAST(a.dom AS VARCHAR2(50))          AS BUILDINGS,
    ...
    CAST(DECODE(d.id_doc_tp, 21, 'RUS', 55, 'ABH', '???') AS VARCHAR2(50)) AS DOCUMENT_TYPE,
...
FROM
    CUS, CUS_DOCUM d, V_OK_SM v, cus_addr a, ISIMPLE_OV_UNIS_CUS u
WHERE d.icusnum (+)= cus.icusnum
  AND d.pref = 'Y'
  AND v.calfa_2 (+)= cus.ccuscountry1
  AND a.icusnum (+)= cus.icusnum
  AND a.addr_type (+)= 0
  AND u.icusnum (+)= cus.icusnum
;
